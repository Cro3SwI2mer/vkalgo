...
/**
 * shift array (cyclic) from right to left (elements on the right move to the left)
 */

void revsubarr(vi& v, int left, int right){
...
void solve(){
    int n; ci >> n; int k; ci >> k;
    k %= n;
    vi v(n); ci >> v;
    revsubarr(v, 1, sz(v));
    revsubarr(v, 1, k);
    revsubarr(v, k + 1, sz(v));
    co << v << endl;
}
